...
const Glassmorphism = () => {
  ...
  return (
    <section
      className="h-full w-full bg-cover bg-center z-0 overflow-hidden h-screen w-screen"
      style={{
        backgroundImage:
          playbackState && playbackState.img
            ? `url(${playbackState.img})`
            : `url(${process.env.PUBLIC_URL}/brutalism.jpg)`,
      }}
    >
      <div
        className={`h-full w-full flex flex-col items-center justify-center backdrop-filter backdrop-blur-3xl z-10 transition-all duration-200 ease-in-out ${
          isLightMode ? "bg-white bg-opacity-10" : "bg-black bg-opacity-50"
        }`}
      >
        <div className="h-3/4 w-full flex flex-col items-center justify-center mt-32 lg:mt-24">
              ...
              >
                {prevTrack && isTrackChanging && (
                  <img
                    ...
                    style={{
                      transform:
                        "translateX(-480px) translateZ(-400px) rotateY(45deg)",
                      filter: "blur(8px)",
                    }}
                    src={prevTrack.img}
          ...
          <div className="relative flex flex-col items-center justify-center mt-20 lg:mt-8 text-white drop-shadow-lg">
            <h1 className="text-3xl lg:text-3xl font-light mb-4">
              {playbackState ? playbackState.name : "Song Title"}
            </h1>
            <h2 className="text-lg lg:text-2xl font-light ">
              {playbackState ? playbackState.artist : "Artist Name"}
            </h2>
          </div>
        </div>
        <div className="h-1/4 w-full flex flex-col items-center justify-start bg-transparent">
          ...
          <div className="flex items-center justify-center space-x-4">
            <button
              className={`${
                isLightMode ? "bg-opacity-40" : "bg-opacity-20 "
              } bg-white backdrop-filter backdrop-blur-lg rounded-full p-4 shadow-lg transition-all duration-200 ease-in-out`}
              onClick={prevSong}
            >
              {/* Back button icon */}
              <svg
            ...
            {playbackState && playbackState.isPlaying ? (
              <button
                className="bg-blue-600 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-full p-4 shadow-lg transform-all ease-in-out duration-200"
              ...
              <button
                className="bg-blue-600 backdrop-filter backdrop-blur-lg rounded-full p-4 shadow-lg transform-all ease-in-out duration-200"
          ...
          {isLightMode ? (
            <button
              className="fixed bottom-4 right-4 bg-white bg-opacity-40 backdrop-filter backdrop-blur-lg rounded-full p-4 shadow-lg"
              onClick={lightMode}
            >
              {/* Dark mode icon */}
              <svg
            ...
            <button
              className="fixed bottom-4 right-4 bg-white bg-opacity-20 backdrop-filter backdrop-blur-lg rounded-full p-4 shadow-lg"
              onClick={lightMode}
            >
              {/* Light mode icon */}
              <svg
      ...
      <div className="fixed h-screen z-20">
        <div className="group h-full">
          ...
          {/* Sliding Panel */}
          <div
            className={`fixed top-0 left-0 h-full w-full lg:w-1/5 bg-white bg-opacity-30 backdrop-blur-xl shadow-lg transform transition-transform duration-300 ${
          ...
          >
            <div className="p-8">
              <button
                className={`absolute top-4 right-4 bg-white bg-opacity-30 rounded-full p-2 shadow-lg ${
                  isLightMode ? "bg-opacity-40 " : "bg-opacity-20"
                }`}
              ...
              <h2 className="text-lg lg:text-2xl font-normal mb-4">
                Search Tracks
              </h2>
              <form onSubmit={handleSearch}>
                ...
                <button
                  type="submit"
                  className="w-full p-2 bg-blue-600 bg-opacity-80 backdrop-blur-xl shadow-lg text-white rounded-full"
                >
                  Search
                </button>
              </form>
